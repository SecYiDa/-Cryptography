...
def main():
	...
	print('\033[1;31;40m')
	print strxor(cypherTextsDecoded[4], key)
	print('\033[0m')
	...
	def strxorr(a, b):     # xor two strings of different lengths
		if len(a) > len(b):
			return "".join([chr(x ^ ord(y)) for (x, y) in zip(a[:len(b)], b)])
		else:
			return "".join([chr(x ^ ord(y)) for (x, y) in zip(a, b[:len(a)])])
	for i in xrange(len(cypherTexts)):
		cypherTexts[i] = cypherTexts[i].decode('hex')
	key = [0] * 173
	H = 0
	for a in cypherTexts:
		...
		for b in cypherTexts:
			...
			for k in range(len(c)):
				if c[k] not in letters : continue    #
		...
		for s in range(len(possible_spaces)):
			if possible_spaces[s] < precision: continue
			key[s] = ord(a[s]) ^ 0x20
	key[2] = 110
	key[8] = 203
	result=''
	print("key",key)
	for text in cypherTexts:
		t=strxorr(key,text)
	t = strxorr(key,cypherTexts[4])
	print('\033[1;32;40m')
	print t
	print('\033[0m')
	raw_input()
	block = [[] for _ in range(173)]
	for i in range(173):
		for j in cypherTexts:
			try:
				block[i].append(j[i])
			except:
				pass
